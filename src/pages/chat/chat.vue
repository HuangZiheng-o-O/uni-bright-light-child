    ...
    <view class="main">
      <scroll-view :scroll-y="true" :scroll-top="scrollTop" :scroll-with-animation="true">
        <view v-for="(e,index) in chat" :key="index">
          <view class="left msg" v-if="e.type === 'volunteer'">
            <image :src="'../../static/images/'+'a'+'.png'"></image>
            <view class="nr">
              <view class="neir">
                {{e.content}}
              </view>
              <view class="nt">{{e.callerName}} {{timeDetia(e.startTime)}}</view>
            </view>
          </view>
          <view class="right msg" v-if="e.type === 'child'">
            <image :src="'../../static/images/'+ 'a' +'.png'"></image>
            <view class="nr">
              <view class="neir">
                {{e.content}}
              </view>
              <view class="nt">{{timeDetia(e.startTime)}}</view>
            </view>
          </view>
          <view class="center" v-if="e.id==3">
            <view class="inner">{{e.callerName}} {{e.name}}</view>
          </view>
        </view>
      </scroll-view>
    </view>
...
<script>
import { getMessageHistoryApi, getSingleMessage, sendMessageApi} from "@/api/chatApi";
...
export default {
  setup(){
    ...
    volunteerId = store.volunteerId
    volunteerName = store.volunteerName
  ...
  onShow (){
    ...
    // const requestTask = connectSSERequest({id: 'V001'})
    let chatData = this.chat
    getMessageHistoryApi({id: 'V001'}).then(res => {
    ...
    this.interValTimer = setInterval(()=>{
      getSingleMessage({id: 'V001'}).then(res => {
        if (res.data !== null){
          chatData.push(res.data)
          this.scrollToBottom()
        }
